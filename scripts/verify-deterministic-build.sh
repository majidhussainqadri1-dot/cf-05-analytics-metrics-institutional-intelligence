...
archive="build/dist/CF-05-sabri-analytics-institutional-intelligence-1.0.0-rc.8.zip"
first=$(sha256sum "$archive" | awk '{print $1}')
cp "$archive" /tmp/cf05-first-rc8.zip
rm -rf build/dist
bash scripts/build-package.sh >/dev/null
second=$(sha256sum "$archive" | awk '{print $1}')
test "$first" = "$second"
cmp -s /tmp/cf05-first-rc8.zip "$archive"
