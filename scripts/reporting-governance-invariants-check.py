...
if "strlen($definition['name']) > 190" not in files['DashboardService.php'] or "trim(wp_strip_all_tags($definition['name'])) !== $definition['name']" not in files['DashboardService.php']: e.append('dashboard canonical name can diverge from stored display name')
shortcode=(r/'src/Presentation/InsightsShortcode.php').read_text(encoding='utf-8')
if "sanitize_key((string) $attributes['dashboard'])" in shortcode or "^[a-z][a-z0-9_.-]{2,189}$" not in shortcode: e.append('dashboard shortcode identity normalization diverges from dashboard contract')
